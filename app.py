...
def index():
    if request.method == "POST":
        ...
        if carpark_id != -1 and carpark_id < data_length:
            ...
            return render_template("index2.html", markers=markers, schema=schema, center=center, name=name)
        else:
            return render_template("index2.html", markers=markers, schema=schema)

@app.route('/browse')
def browse():
    carpark_id = int(request.args.get('id', -1))
    if carpark_id != -1 and carpark_id < data_length:
        return render_template("browse.html", data=add_carparks_availability(data, schema), schema=schema, id=carpark_id, name=data[carpark_id][schema["name"]])
    else:
        return render_template("browse.html", data=add_carparks_availability(data, schema), schema=schema)
...
def browse2():
    ...
    if carpark_id != -1 and carpark_id < data_length:
        center = data[carpark_id][schema["location"]]
        return render_template("browse2.html", markers=markers, id=carpark_id, center=center, name=data[carpark_id][schema["name"]])
    else:
        return render_template("browse2.html", markers=markers)
